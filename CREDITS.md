# Credits

**Hey Voice was created by StefanoGPT — [@StefanoGPT](https://x.com/StefanoGPT).**

Concept, product direction, and testing by StefanoGPT. Development and artwork were assisted by Codex. Thanks to everyone contributing fixes, translations, testing, and ideas.

The source is released under the [MIT license](LICENSE). See [artwork details](docs/ARTWORK.md) for the orb asset and generation prompt.

Hey Voice is an independent community project. It is not affiliated with or endorsed by OpenAI or Apple.
